        ...
        <main  className='p-2 md:p-3 mt-10'>
            <div className="container mx-auto text-white md:px-5">
                <div data-aos="fade-up" data-aos-duration="1000" className="grid md:grid-cols-2 gap-x-2 gap-y-10 md:gap-10 mb-5">
                    {data?.map((item, index) => (
                        <Link data-aos="fade-up" data-aos-duration="1000" key={item.id} href={`/services/${item.id}`} className={`hover:scale-90 transition-all duration-500 ${index + 1 === data.length && data.length % 2 !== 0 ? "md:col-span-2" : ""}`}>
